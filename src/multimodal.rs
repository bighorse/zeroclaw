...
pub async fn prepare_messages_for_provider(
    ...
    // When image count exceeds the limit, strip markers from the oldest
    // messages first so that the most recent (most relevant) images survive.
    // This prevents conversations from becoming permanently stuck once the
...
/// image count is within `max_images`. Text content of each message is kept.
fn trim_old_images(messages: &[ChatMessage], max_images: usize) -> Vec<ChatMessage> {
    // Positions of user messages that carry images, oldest first.
    ...
    // How many images must be dropped, starting from the oldest messages.
...
mod tests {
    ...
    #[test]
    fn trim_old_images_strips_oldest_when_over_limit() {
        // 3 messages, one image each, limit 2 -> the oldest image is stripped.
        let messages = vec![
    ...
    fn trim_old_images_replaces_image_only_message() {
        // A message with only an image and no text gets a placeholder.
        let messages = vec![
    ...
    fn trim_old_images_multi_image_message_stripped_as_unit() {
        // Trimming works at message granularity: a message holding 3 images
        // loses all of them, which over-trims below the limit. Accepted
        // trade-off -- the newest messages keep their images.
        let messages = vec![
            ChatMessage::user(
                "[IMAGE:/tmp/a.png]\n[IMAGE:/tmp/b.png]\n[IMAGE:/tmp/c.png]\nThree pics"
                    .to_string(),
    ...
    fn trim_old_images_skips_assistant_messages() {
        // Assistant messages are neither counted nor stripped.
        let messages = vec![
            ChatMessage {
    ...
    #[test]
    fn trim_old_images_handles_empty_history() {
    ...
    fn trim_old_images_interleaved_roles() {
        // Realistic conversation: user image, assistant reply, user image, ...
        let messages = vec![
            ...
            ChatMessage {
                role: "assistant".to_string(),
                content: "That is a chart.".to_string(),
            },
        ...
        assert_eq!(trimmed[3].content, "That is a chart.");
    ...
    fn trim_old_images_strips_multiple_oldest_messages() {
        // 5 user images, limit 1 -> the first four messages lose their images.
    ...
    async fn prepare_messages_trims_then_normalizes_surviving_images() {
        // End-to-end: 3 images, limit 2. The oldest is trimmed and the two
        // survivors are still normalized into data URIs -- no error.
        let temp = tempfile::tempdir().unwrap();
        let mut paths = Vec::new();
        for name in ["old.png", "mid.png", "new.png"] {
            let image_path = temp.path().join(name);
            // Minimal PNG signature bytes are enough for MIME detection.
            std::fs::write(
                &image_path,
                [0x89, b'P', b'N', b'G', b'\r', b'\n', 0x1a, b'\n'],
        ...
        let prepared = prepare_messages_for_provider(&messages, &config)
            .await
            .expect("should succeed after trimming oldest images");

        assert!(prepared.contains_images);
        assert_eq!(prepared.messages.len(), 3);
        assert!(!prepared.messages[0].content.contains("data:image"));
        assert!(prepared.messages[0].content.contains("Old"));
        assert!(prepared.messages[1].content.contains("data:image"));
        assert!(prepared.messages[2].content.contains("data:image"));
    }
}
