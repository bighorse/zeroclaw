        ...
        if let Some(caps) = approve_re.captures(head) {
            ...
            return match result {
                Ok(action) => {
                    ...
                    // 审核人在前台改写、或要求终稿删掉的句子,批准时一并交给助手——
                    // 与网关 /sop/approve 读的是同一份逐条记录(state 下的 verdicts.jsonl),
                    // 拼段落也是同一个函数,两条批准路径交给助手的要求一字不差。
                    // 不带上它的话,「先在前台改、再在飞书批准」这条路径上,改写与删除
                    // 都不会进终稿,前台随后只会显示「终稿里找不到这句」「这一条还在」,
                    // 看不出原因是走了飞书批准。
                    // 没配 workspace_dir 时读不到记录,这里如实不加,不假装带上了改动。
                    let edits_note = self
                        .workspace_dir
                        .as_ref()
                        .and_then(|ws| {
